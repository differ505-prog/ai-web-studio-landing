...
- 驗證：Auth.js + Google OAuth + Email 白名單
- 資料層：目前以 mock data + Vercel KV 保存簽署短連結與已簽留存；後續可升級 PostgreSQL + ORM
- 簽署儲存：Vercel KV 保存簽署 token、最終快照、簽署紀錄與索引
...
|-------|---------|
| /login | 築時管理者登入頁 |
| /api/auth/[...nextauth] | Google 登入與 callback |
| /studio | 內部總覽儀表板 |
...
| /api/auth/[...nextauth] | GET / POST | 處理 Google 登入、callback 與 session |
